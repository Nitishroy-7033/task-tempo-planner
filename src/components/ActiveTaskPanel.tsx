...
type CountdownStyle = 'digital' | 'circle' | 'minimal' | 'neon';
...
export const ActiveTaskPanel = ({ isFullscreen = false, onToggleFullscreen }: ActiveTaskPanelProps) => {
  const [isRunning, setIsRunning] = useState(false);
  const [timeRemaining, setTimeRemaining] = useState(7200); // 2 hours in seconds
  const [isCompleted, setIsCompleted] = useState(false);
  const [countdownStyle, setCountdownStyle] = useState<CountdownStyle>('digital');
  ...
  return (
    <Card className={cardClass}>
      ...
      <CardContent className={`flex flex-col items-center justify-center h-full ${isFullscreen ? 'p-12 space-y-12' : 'p-6 space-y-8'}`}>
        <div className="text-center space-y-4">
        ...
        <div className={`flex space-x-4 ${isFullscreen ? 'scale-125' : ''}`}>
          {!isRunning ? (
            <Button
              onClick={handleStart}
